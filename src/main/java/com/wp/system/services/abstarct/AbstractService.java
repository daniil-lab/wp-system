...
public class AbstractService {
    ...
    public PagingResponse<Object> getAllTransactions(Instant startDate, Instant endDate, int page, int pageSize) {
        ...
        Query query = entityManager.createNativeQuery("""
                ((select CAST(id as varchar), CAST(category_id as varchar), sum, action, create_at, currency, description, 'SYSTEM' as transaction_type from bill_transaction WHERE create_at BETWEEN :startDate AND :endDate AND user_id = :userId)
                union
                ...
                ) order by create_at desc limit :limit offset :offset""", HashMap.class);
